...
public class Rule_biquzw789 {
    public static void addToThis(String ID) throws IOException {
        InitSrcCopy(ID);
        Dw_File(GetDownloadURL(ID), TempPath+"/tmp-dw.txt");
        Dw_File(GetImage(), TempPath+"/tmp.jpg");
        autoMake(TempPath+"/tmp-dw.txt", TempPath+"/tmd-dw.zip", GetTitle(), "tmp.jpg", "", GetAuthor(), GetInfo());
        Unzip(TempPath+"/tmd-dw.zip", Config_dirs.MainPath);
