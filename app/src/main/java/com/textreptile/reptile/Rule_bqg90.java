...
public class Rule_bqg90 {
    ...
    public static void addToThis(String ID) throws IOException {
        InitSrcCopy(ID);
        Dw_File(GetImage(), TempPath+"/tmp.jpg");
        //1.获取目录列表
        ...
        String Img = GetImage();
        //2.下载
        File file_txt = new File(TempPath+"/tmp_main_txt.txt");
        if (file_txt.exists()) {
            file_txt.delete();
        ...
        //3.封装-加入
        userMake(TempPath+"/tmp_main_txt.txt", TempPath+"/tmp-dw.zip", Title, TempPath+"/tmp.jpg", index.toString(), Aunthor, Info);
    ...
    public static String Search(String key_word) throws MalformedURLException {
        //逆天!放着接口每个保护.放着让人爬!!
        Dw_File("https://www.bqg90.com/user/hm.html?q=",TempPath+"/tmp-dw-req.txt");
        Dw_File("https://www.bqg90.com//user/search.html?q=",TempPath+"/tmp-dw-req.txt");
...
//https://www.bqg90.com/user/hm.html?q=%E5%B9%B4
    ...
    public static void InitSrcCopy(String ID) throws MalformedURLException {
        Dw_File(GetURL(ID), TempPath+"/tmp-hb.txt");
    }
    ...
    public static List<String> GetMainText(String URL) throws MalformedURLException {
        Dw_File(URL, TempPath+"/tmp-tx.txt");
        List<String> lines = ReadCFGFile(TempPath+"/tmp-tx.txt");
        for (String line : lines) {
    ...
    public static String GetImage() throws MalformedURLException {
        List<String> lines = ReadCFGFile(TempPath+"/tmp-hb.txt");
        for (String line : lines) {
